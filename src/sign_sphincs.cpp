...
[[cpp11::register]]
cpp11::raws cpp_sign_sphincs_sha2(cpp11::raws message,
                                  cpp11::raws private_key,
                                  bool is_fast) {
  ...
  case 64:
    if (is_fast) {
      sign = new uint8_t[SPX_SHA128F_CRYPTO_BYTES];
      result = SPX_SHA128F_crypto_sign_signature(sign, &sign_len, msg, msg_len, prvt_k);
    } else {
      sign = new uint8_t[SPX_SHA128S_CRYPTO_BYTES];
      result = SPX_SHA128S_crypto_sign_signature(sign, &sign_len, msg, msg_len, prvt_k);
    }
    break;

  case 96:
    if (is_fast) {
      sign = new uint8_t[SPX_SHA192F_CRYPTO_BYTES];
      result = SPX_SHA192F_crypto_sign_signature(sign, &sign_len, msg, msg_len, prvt_k);
    } else {
      sign = new uint8_t[SPX_SHA192S_CRYPTO_BYTES];
      result = SPX_SHA192S_crypto_sign_signature(sign, &sign_len, msg, msg_len, prvt_k);
    }
    break;

  case 128:
    if (is_fast) {
      sign = new uint8_t[SPX_SHA256F_CRYPTO_BYTES];
      result = SPX_SHA256F_crypto_sign_signature(sign, &sign_len, msg, msg_len, prvt_k);
    } else {
      sign = new uint8_t[SPX_SHA256S_CRYPTO_BYTES];
      result = SPX_SHA256S_crypto_sign_signature(sign, &sign_len, msg, msg_len, prvt_k);
    }
    break;
  }
